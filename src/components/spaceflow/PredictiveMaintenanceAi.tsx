...
export const PredictiveMaintenanceAi: React.FC = () => {
  ...
  const [chatMessages, setChatMessages] = useState<Array<{ sender: 'user' | 'ai'; text: string; time: string }>>([
    {
      sender: 'ai',
      text: 'Hello! I am your Facility & Predictive Maintenance AI Assistant. Which equipment would you like to diagnose or schedule today?',
      time: '11:42',
    },
  ]);
  ...
  const handleDispatchWorkOrder = (anomaly: AnomalyItem) => {
    ...
    addWorkOrder({
      ...
      priority: anomaly.riskScore > 80 ? 'High' : 'Medium',
      status: 'To Do',
      dueDate: 'Today 17:00',
      equipmentId: anomaly.equipmentId,
      description: `Automatically generated by Predictive AI Engine. Diagnosis: ${anomaly.cause}`,
    });
  ...
  const handleSendChatMessage = async (e?: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/bizos/ask', {
        ...
        body: JSON.stringify({
          prompt: userText,
          context: 'BizOS GMAO / Predictive Maintenance & Equipment Diagnostics'
        })
      });
    ...
    let aiResponse = "Telemetry analysis confirms nominal performance across all 128 sensors. No critical anomalies detected.";
    
    const lower = userText.toLowerCase();
    if (lower.includes('elevator') || lower.includes('ascenseur') || lower.includes('elev')) {
      aiResponse = "The North elevator drive (ELEV-01) exhibits a temperature of 62.4°C (Risk Score 88%). A priority work order has been pre-generated for the FieldTech mobile team.";
    } else if (lower.includes('pump') || lower.includes('pompe') || lower.includes('vibration')) {
      aiResponse = "Pump P-02 shows a vibration drift of 3.8 mm/s at 120 Hz. The predictive model recommends SKF greasing within 7 days to prevent seizure.";
    } else if (lower.includes('part') || lower.includes('stock') || lower.includes('filter')) {
      aiResponse = "Spare parts (SKF-6204 bearings, HVAC G4 filter kit, Schneider Altivar drive) are in stock at Warehouse -1 (3 units available).";
    } else if (lower.includes('esg') || lower.includes('energy') || lower.includes('kwh')) {
      aiResponse = "The building records a Class B overall energy efficiency. Class A BMS regulation reduces HVAC consumption by 22% this month.";
    } else if (lower.includes('wo') || lower.includes('order') || lower.includes('interv') || lower.includes('work')) {
      aiResponse = "You can click 'Create Work Order WO' to assign the job directly to the technicians' FieldTech Mobile App.";
    }
                  ...
                  <div className="flex items-center justify-between pt-1">
                    ...
                    {anom.status === 'Dispatched' ? (
                      ...
                      >
                        ...
                        <span>Create Work Order WO</span>
                      </button>
                    )}
                  </div>
                </div>
